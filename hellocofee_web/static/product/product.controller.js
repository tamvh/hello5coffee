...
(function () {
    ...
    function ProductController($scope, ProductService, $rootScope, $cookies, $location, PopupService, $uibModal) {
        ...
        $scope.tablenumber = "#";
        $scope.tablelocation = "#";
        $scope.dheight = $(window).height();
        $scope.current_category = "Tất cả";
        $scope.styleloading = "margin-top:" + ($(window).height() / 2 - 150) + "px;";
        ...
        function get_params() {
            ...
            if (cur_url.split('?').length === 2) {
                ...
                if (params.split('=').length === 2) {
                    ...
                    $rootScope.merchant_code = json_obj.merchant_code;
                }
            }
        }
        ...
        $scope.getProdInCategory = function (cate) {
            ...
            if (cate_name === "Tất cả") {
                ...
                for (var i in $rootScope.l_categories) {
                    var cate_value = $rootScope.l_categories[i].category_value;
        ...
        $scope.getListProduct = function () {
            ...
            var count_item_in_cate = 0;
            var date = new Date();
            var current_day_no = date.getDay();
            var day_name = "";
            console.log('current date ' + current_day_no);
            ...
            ProductService.getListProduct($rootScope.merchant_code)
                        ...
                        if (response.err === 0) {
                            ...
                            for (var i in response.dt.categories) {
                                ...
                                if (response.dt.categories[i].status === 1) {
                                    if (response.dt.categories[i].category_name.toString() === day_name) {
                                        response.dt.categories[i].category_name = "Món ăn";
                                    }
                                    response.dt.categories[i].category_color = "#000000";
                                    response.dt.categories[i].icon = "img/menu_item.svg";
                                    $rootScope.l_categories.push(response.dt.categories[i]);
                                    var cate_value = response.dt.categories[i].category_value;
                                    for (var i in dt_items) {
                                        if ($scope.checkMask(dt_items[i].cate_mask, cate_value)) {
                                            if (dt_items[i].status === 1) {
                                                count_item_in_cate = count_item_in_cate + 1;
